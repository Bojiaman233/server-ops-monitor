...
- Nginx 访问日志分析：PV（总请求数）、独立 IP 数、HTTP 状态码分布（200 / 404 / 500）
...
├── schema.sql        数据库建表语句（patrol_log / web_stat）
├── .gitignore        忽略运行时日志与生成文件
├── .last_line        web_collect.sh 增量位点（git 忽略）
...
- `ops.web_stat`：Web 访问统计结果表，每 5 分钟一个窗口（总请求数 / 各状态码计数 / 窗口内独立 IP 数）
